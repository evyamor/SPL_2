...
public abstract class MicroService implements Runnable {
    ...
    private final MessageBusImpl msgBus;
    private boolean terminated = false;
    private final ConcurrentHashMap<Class<? extends Message>, Callback> messagesCallBack;
    ...
    public MicroService(String name) {
        ...
        messagesCallBack = new ConcurrentHashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        msgBus.subscribeEvent(type,this); //notify messageBus this MicroService can handle this kind of Event
        messagesCallBack.put(type,callback); //stores a callback function to this kind of message
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
    	msgBus.subscribeBroadcast(type,this); //notify messageBus this MicroService can handle broadcast
    	messagesCallBack.put(type,callback);//stores a callback function for broadcast
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return this.msgBus.sendEvent(e);
    }
    ...
    protected final void terminate() {
        System.out.println("Im done ! "+ this.getName());
    ...
    public final void run() {
        ...
        initialize();
        while (!terminated){ //as long as the microservice was not terminated
            try {
                Message message = msgBus.awaitMessage(this); //will keep trying getting messages
                messagesCallBack.get(message.getClass()).call(message); // make callback on them
            }
    	...
    	msgBus.unregister(this); //when finishes and terminated, we unregister it from the messageBus
